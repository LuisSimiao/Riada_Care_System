// Synthetic environment readings for CARE-B-RM2

function getCARE_B_RM2_Readings() {
  const readings = [];
  const now = new Date();
  for (let h = 0; h < 24; h++) {
    const timestamp = new Date(now.getFullYear(), now.getMonth(), now.getDate(), h, 0, 0, 0);
    readings.push({
      device_id: "CARE-B-RM2",
...
function getCARE_B_RM2_ReadingNow() {
  const now = new Date();
...
module.exports = { getCARE_B_RM2_Readings, getCARE_B_RM2_ReadingNow };